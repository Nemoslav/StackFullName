...
{
  Node *nod = new Node{value, back_ }; // создаем новое звено, предыдущее для которого - последнее на данный момент звено

    if (nod == nullptr) // если память под новое звено не выделилась
    {
        throw EStackException("Lack of memory."); // возвращаем исключение, которое говорит о том что память закончилась
    }

    back_ = nod; // делаем только что созданное звено последним
